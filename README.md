...
- 使用 React + TypeScript 构建，提高可维护性
- 严格的 CSP 与安全响应头，默认无缓存
...
### 人机验证：
前端登录/注册请求会附带 `X-CF-Turnstile` 头，后端验证 `TURNSTILE_SECRET`。
...
### SQL 注入防护：
所有数据库操作采用 `prepare(...).bind(...)` 预编译语句，无字符串拼接。
...
### 更新日志
参见 `CHANGELOG.md` 了解最近的变更和部署上下文。

### 典型流程
1. 配置 `wrangler.toml` 中的 `[vars]`（例如 `CUSTOM_DOMAIN`、`TURNSTILE_SITE_KEY`）。
2. 通过 `wrangler secret put` 设置密钥（`TURNSTILE_SECRET`、`JWT_SECRET`）。
3. 使用 `wrangler deploy` 部署。
4. 验证 `/api/config` 返回 sitekey/domain；测试带有 Turnstile 的身份验证。
